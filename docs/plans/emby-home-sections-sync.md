...
- **Targets are `is_enabled AND NOT provider_disabled` for BOTH rows** (the plan widened Top Picks to admins); non-targets have their rows removed. Switching the feature off is the same run with nothing desired.
