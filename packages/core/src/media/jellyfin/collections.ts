/**
 * Jellyfin Collections (Box Sets) Module
 * 
...
export async function createOrUpdateCollection(
  ...
  const existing = await provider.fetch<JellyfinItemsResponse>(`/Items?${params}`, apiKey)
  ...
  if (collection) {
    ...
    const existingItems = await provider.fetch<JellyfinItemsResponse>(
...
async function setCollectionSortTitle(
  ...
  try {
    // First, fetch the full item data (required by Jellyfin API for updates)
