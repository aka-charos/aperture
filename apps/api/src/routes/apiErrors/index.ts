...
const apiErrorsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: { provider: string } }>(
    ...
    async (request, reply) => {
      ...
      // Validate provider
      const validProviders = ['openai', 'google', 'tmdb', 'trakt', 'mdblist', 'omdb']
      if (!validProviders.includes(provider)) {
        return reply.status(400).send({ error: 'Invalid provider' })
      }
      
      try {
        const errors = await getActiveErrorsByProvider(provider as 'openai' | 'google' | 'tmdb' | 'trakt' | 'mdblist' | 'omdb')
  ...
  fastify.post<{ Params: { provider: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        const count = await dismissErrorsByProvider(provider as 'openai' | 'google' | 'tmdb' | 'trakt' | 'mdblist' | 'omdb')
