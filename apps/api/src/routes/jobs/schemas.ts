...
export const jobComponentSchemas = {
  ...
  JobProgress: {
    ...
    properties: {
      ...
      startedAt: { type: 'string' as const, format: 'date-time', description: 'When the job started' },
      completedAt: { type: 'string' as const, format: 'date-time', nullable: true, description: 'When the job completed' },
      currentStep: { type: 'string' as const, description: 'Current step name' },
      currentStepIndex: { type: 'integer' as const, description: 'Current step index (0-based)' },
      totalSteps: { type: 'integer' as const, description: 'Total number of steps' },
      stepProgress: { type: 'number' as const, description: 'Progress within current step (0-100)' },
      overallProgress: { type: 'number' as const, description: 'Overall progress percentage (0-100)' },
      itemsProcessed: { type: 'integer' as const, description: 'Items processed so far' },
      itemsTotal: { type: 'integer' as const, description: 'Total items to process' },
      ...
      logs: {
        ...
        items: {
          ...
          properties: {
            ...
            level: { type: 'string' as const, enum: ['info', 'warn', 'error', 'debug'] },
      ...
      result: { type: 'object' as const, nullable: true, description: 'Job result data' },
    },
  },
} as const
